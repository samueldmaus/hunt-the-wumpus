...
// Custom Assert if we wanted to use a macro
#define MY_CUSTOM_ASSERT(BoolCondition) do {if (!(BoolCondition)) {throw std::logic_error("Bool expression is false"); }} while(0)
	...
	while (change.IsPlaying())
	{
		...
		if (command == "m" || command == "M" || command == "move" || command == "MOVE")
		{
			...
			try
			{
				...
				// showing example of how my custom assert macro would work in comments (does same as the if block)
				//MY_CUSTOM_ASSERT(destCave < 20 && destCave > 0);
				if (destCave > 20 || destCave < 1)
				{
					throw std::out_of_range("Cave number identifier must be between 1 and 20");
				}
		...
		else if (command == "s" || command == "S" || command == "shoot" || command == "SHOOT")
		{
			if (stringTokens.size() < 2)
			{
			...
			for (auto&& token : stringTokens)
			{
				...
				try
				{
					if (std::stoi(token) > 20 || std::stoi(token) < 1)
					{
						throw std::out_of_range("Cave number identifier must be between 1 and 20");
					}
		...
		else if (command == "q" || command == "quit" || command == "e" || command == "exit" || command == "x")
		{
		...
		{
			std::cout << "Please enter a valid command(move ..., shoot ..., quit)." << std::endl;
		}
	}
